...
public class GridSystem<TGridObject>
{
    ...
    public GridSystem(int width, int height, float cellSize, Vector3 originPosition)
    {
        ...
        for (int x = 0; x < gridArray.GetLength(0); x++)
        {
            ...
            {
                debugTextArray[x, y] = Utils.CreateWorldText(gridArray[x, y].ToString(), null, GetWorldPosition(x, y) + new Vector3(cellSize, cellSize) * .5f, 10);
    ...
    public TGridObject GetObjectWithWorldPosition(Vector3 worldPosition)
    {
        int x, y;
        GetXY(worldPosition, out x, out y);
